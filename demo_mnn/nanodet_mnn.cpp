...
NanoDet::NanoDet(
  ...
  NanoDet_interpreter = std::shared_ptr<MNN::Interpreter>(
...
int NanoDet::detect(cv::Mat& raw_image, std::vector<BoxInfo>& result_list) {
  ...
  NanoDet_interpreter->resizeTensor(
      input_tensor, {1, 3, input_size[0], input_size[1]});
  ...
  pretreat->convert(
      image.data, input_size[1], input_size[0], image.step[0], input_tensor);
  ...
  MNN::Tensor* tensor_preds = NanoDet_interpreter->getSessionOutput(
      NanoDet_session, output_name.c_str());
  ...
  generate_grid_center_priors(
      this->input_size[0], this->input_size[1], this->strides, center_priors);
  ...
  for (int idx = 0; idx < num_points; idx++) {
    ...
    if (score > threshold) {
      const float* bbox_pred =
          pred->host<float>() + idx * num_channels + num_class;
