...
std::vector<BoxInfo> NanoDet::detect(
    cv::Mat image,
  ...
  {
    const InferenceEngine::Blob::Ptr pred_blob =
        infer_request_.GetBlob(output_name_);
  ...
  // double time = std::chrono::duration<double, std::milli>(end -
  // start).count(); std::cout << "inference time:" << time << "ms" <<
  // std::endl;
  return dets;
}
...
void NanoDet::decode_infer(
  ...
  for (int idx = 0; idx < num_points; idx++) {
    ...
    if (score > threshold) {
      // std::cout << row << "," << col <<" label:" << cur_label << " score:" <<
      // score << std::endl;
