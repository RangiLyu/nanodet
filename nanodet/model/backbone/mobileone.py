...
class MobileOne(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """ Apply forward pass. """
        output = []
        output_stage0_x = self.stage0(x)
        output_stage1_x = self.stage1(output_stage0_x)
        output_stage2_x = self.stage2(output_stage1_x)
        output_stage3_x = self.stage3(output_stage2_x)
        #output_stage4_x  = self.stage4(output_stage3_x)
        #x_out = self.gap(output_stage4_x)
        #x_out = x_out.view(x_out.size(0), -1)
        #x_out = self.linear(x_out)
        output.append(output_stage1_x) 
        output.append(output_stage2_x) 
        output.append(output_stage3_x) 
        #output.append(output_stage4_x) 
        #print("**************the shape of stage1,stage2,stage3,stage4:******************")
        #print(output_stage1_x.shape)
        #print(output_stage2_x.shape)
        #print(output_stage3_x.shape)
        #print(output_stage4_x.shape)
        return tuple(output) 
...
def conv_operator_enhancement(x, kernel, in_channels=1):
    # if in_channels == 1:
    ...
    return y
def sobel_x_enhancement(x, in_channels, kernel=sobel_x):
    return conv_operator_enhancement(x, kernel, in_channels=in_channels)

def sobel_y_enhancement(x, in_channels, kernel=sobel_y):
    return conv_operator_enhancement(x, kernel, in_channels=in_channels)

def laplace_enhancement(x, in_channels, kernel=laplace):
    return conv_operator_enhancement(x, laplace, in_channels=in_channels)
